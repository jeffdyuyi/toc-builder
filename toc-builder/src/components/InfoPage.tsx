...
export default function InfoPage({ data, setData, showOccupations, setShowOccupations, showDrives, setShowDrives, showPillars, setShowPillars }: InfoPageProps) {
    ...
    const openInfo = (title: string) => {
        const content = SKILL_DESCRIPTIONS[title];
        if (content) {
    ...
    const hitThreshold = athleticLevel >= 8 ? 4 : 3;
    ...
    return (
        <>
            ...
            {
                infoModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40" onClick={() => setInfoModal(null)}>
                        <div className="bg-[#faf8f2] border-[2px] border-[#daaa39] max-w-md w-full p-5 rounded-sm shadow-xl" onClick={e => e.stopPropagation()}>
                            <div className="flex justify-between items-center border-b border-[#daaa39] pb-2 mb-3">
                                <h3 className="font-bold text-[#5c4a21] text-lg tracking-widest">{infoModal.title}</h3>
                                <button className="text-[#c89b3c] hover:text-[#8b6d2a] font-bold text-xl leading-none" onClick={() => setInfoModal(null)}>×</button>
                            </div>
                            <div className="text-slate-800 space-y-2 text-[13px] leading-relaxed font-serif whitespace-pre-wrap max-h-[60vh] overflow-y-auto">
                                {infoModal.content}
                            </div>
                        </div>
                    </div>
